	...
	public static class IpcUtilEx
	{
		...
		public static void SendGlobalMessage(IpcParamEx ipcMsg, bool bOneInstance)
		{
			...
			try
			{
				...
				IpcBroadcast.Send(nId, true, bOneInstance ? Program.AppMessage.IpcByFile1 :
					Program.AppMessage.IpcByFile);
			}
		...
		private static string GetIpcFilePath(int nId)
		{
			try
			{
				string str = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.Util.IpcUtilEx");
		...
		{
			ProcessGlobalMessage(true, nId, mf); // Backward compatibility
		}

		public static void ProcessGlobalMessage(bool bOneInstance, int nId, MainForm mf)
		{
