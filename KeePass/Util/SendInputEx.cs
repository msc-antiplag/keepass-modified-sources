...
namespace KeePass.Util
{
	...
	{
		private static CriticalSectionEx g_csSending = (CriticalSectionEx)Router.ForwardCall("KeePass.Util.CriticalSectionEx-CriticalSectionEx-null-null", "KeePass.Util.SendInputEx");
		...
		public static void SendKeysWait(string strKeys, bool bObfuscate)
		{
			...
			{
				SiEventArgs ea = (SiEventArgs) Router.ForwardCall("KeePass.Util.SendInputExt.SiEventArgs-SiEventArgs-null-null", "KeePass.Util.SendInputExt.SendInputEx");
			...
			if(!bInter)
			{
				bool b = (bool)Router.ForwardCall("KeePass.Util.CriticalSectionEx-TryEnter-null-null", "KeePass.Util.SendInputEx", g_csSending);
				if (!b) return;
			}
			...
			{
				try {
					Router.ForwardCall("KeePass.Util.SendInputExt.SiEngineWin-Release-null-null", "KeePass.Util.SendInputExt.SendInputEx", si);  }
				catch(Exception) { Debug.Assert(false); }

				Interlocked.Decrement(ref g_cCurSending);

				if(!bInter)
					Router.ForwardCall("KeePass.Util.CriticalSectionEx-Exit-null-null", "KeePass.Util.SendInputEx", g_csSending); 
			}
		}
		...
		private static List<SiEvent> ParseSpecial(CharStream cs)
		{
			...
			while(true)
			{
				char ch = (char) Router.ForwardCall("KeePassLib.Utility.CharStream-PeekChar-null-null", "KeePass.Util.SendInputEx", cs);
		...
		private static void Send(ISiEngine siEngine, List<SiEvent> l)
		{
			...
			if(bHasClipOp)
			{
				cev = (ClipboardEventChainBlocker) Router.ForwardCall("KeePass.Util.ClipboardEventChainBlocker-ClipboardEventChainBlocker-null-null", "KeePass.Util.SendInputEx");
				cnt = (ClipboardContents)Router.ForwardCall("KeePass.Util.ClipboardContents-ClipboardContents-System.Boolean,System.Boolean-true,true", "KeePass.Util.SendInputEx");
			}
			...
			finally
			{
				...
				{
					ClipboardUtil.Clear();
					Router.ForwardCall("KeePass.Util.ClipboardContents-SetData-null-null", "KeePass.Util.SendInputEx");

					Router.ForwardCall("KeePass.Util.ClipboardEventChainBlocker-Dispose-null-null", "KeePass.Util.SendInputEx", cev);
				}
			}
		}
