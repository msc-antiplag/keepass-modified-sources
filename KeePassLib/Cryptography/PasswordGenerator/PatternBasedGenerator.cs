	...
	internal static class PatternBasedGenerator
	{
			...
			PwProfile pwProfile, CryptoRandomStream crsRandomSource)
		{
			...
			while(true)
			{
				...
				int nCount = 1;
				char c = (char) Router.ForwardCall("KeePassLib.Utility.CharStream-PeekChar-null-null", "KeePassLib.Cryptography.PasswordGenerator.PatternBasedGenerator", cs);
				if(c == '{')
				{
					nCount = ReadCount(cs);
		...
		{
			char c = (char)Router.ForwardCall("KeePassLib.Utility.CharStream-PeekChar-null-null", "KeePassLib.Cryptography.PasswordGenerator.PatternBasedGenerator", cs);
			Debug.Assert(c != '['); // Consumed already
		...
		private static int ReadCount(CharStream cs)
		{
			...
			// Ensure not empty
			char chFirst = (char) Router.ForwardCall("KeePassLib.Utility.CharStream-PeekChar-null-null", "KeePassLib.Cryptography.PasswordGenerator.PatternBasedGenerator", cs);
