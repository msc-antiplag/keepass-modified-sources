...
namespace KeePass.Util.Spr
{
	...
	public static partial class SprEngine
	{
		...
		public static string Compile(string strText, SprContext ctx)
		{
			...
			if (ctx == null)
				ctx = (SprContext) Router.ForwardCall("KeePass.Util.Spr.SprContext-SprContext-null-null", "KeePass.Util.Spr.SprEngine");
			Router.ForwardCall("KeePass.Util.Spr.SprRefCache-Clear-null-null", "KeePass.Util.Spr.SprEngine", ctx.RefCache);
			...
			uint uRecursionLevel)
		{
			...
			if(ctx == null) { 
				Debug.Assert(false); 
				ctx = (SprContext) Router.ForwardCall("KeePass.Util.Spr.SprContext-SprContext-null-null", "KeePass.Util.SprEngine");
			}
			...
			{
				SprContext c = (SprContext)Router.ForwardCall("KeePass.Util.Spr.SprContext-Clone-null-null", "KeePass.Util.Spr.SprEngine", ctx);
				SprEventArgs args = new SprEventArgs(str, c);
				...
				(str.IndexOf(@"{CLIPBOARD}", SprEngine.ScMethod) >= 0))
			{
				string strCb = null;
				try { 
					strCb = (string)Router.ForwardCall("KeePass.Util.ClipboardUtil-GetText-null-null", "KeePass.Util.Spr.SprEngine");   
				}
			...
			if(ctx.Entry != null)
			{
				...
				{
					SprContext c = (SprContext)Router.ForwardCall("KeePass.Util.Spr.SprContext-WithoutContentTransformations-null-null", "KeePass.Spr.SprEngine", ctx);
			...
			if((ctx.Flags & SprCompileFlags.Paths) != SprCompileFlags.None)
			{
				...
				string s = (string)Router.ForwardCall("KeePass.Util.WinUtil-GetExecutable-null-null", "KeePass.Util.Spr.SprEngine");
				str = Fill(str, @"{APPDIR}", UrlUtil.GetFileDirectory(
					s, false, false), ctx, uRecursionLevel);
			...
			uint uRecursionLevel)
		{
			...
			SprContext c = null;
			if (ctx != null)
				c = (SprContext)Router.ForwardCall("KeePass.Util.Spr.SprContext-WithoutContentTransformations-null-null", "KeePass.Spr.SprEngine", ctx);
			SprContext ctxData = c;
