		...
		private static void ExecuteShellCmd(EcasAction a, EcasContext ctx)
		{
			...
			try
			{
				...
				if((p != null) && bWait)
				{
					...
					Router.ForwardCall("KeePassLib.Utility.MessageService-ExternalIncrementMessageCount-null-null", "KeePass.Ecas.EcasDefaultActionProvider");

					try { p.WaitForExit(); }
					catch(Exception) { Debug.Assert(false); }

					Router.ForwardCall("KeePassLib.Utility.MessageService-ExternalDecrementMessageCount-null-null", "KeePass.Ecas.EcasDefaultActionProvider");
