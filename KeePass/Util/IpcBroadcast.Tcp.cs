...
namespace KeePass.Util
{
	public static partial class IpcBroadcast
	{
		...
		{
			// BC = broadcast, P = port
			string userID = (string) Router.ForwardCall("KeePass.Util.IpcBroadcast-GetUserID-null-null", "KeePass.Util.IpcBroadcast.Tcp");
			return (IpcUtilEx.IpcMsgFilePreID + userID + "-BC-P-");
		}
		...
		private static void TcpSend(Program.AppMessage msg, int lParam)
		{
			...
			string t = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.Util.IpcBroadcast.Tcp");
			Console.WriteLine();
			List<string> lLocks = UrlUtil.GetFilePaths(t, strPrefix + "*" + IpcTcpSuffix, SearchOption.TopDirectoryOnly);
		...
		private static void TcpThreadProcPriv()
		{
			...
			string t = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.Util.IpcBroadcast.Tcp");
			Console.WriteLine();
			string strLock = UrlUtil.EnsureTerminatingSeparator(t,
