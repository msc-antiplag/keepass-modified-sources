	...
	public static class PlgxPlugin
	{
			...
			IStatusLogger slStatus)
		{
			...
			while(true)
			{
				...
				{
					ulong uReq = MemUtil.BytesToUInt64(kvp.Value);
					ulong uInst = (ulong) Router.ForwardCall("KeePass.Util.WinUtil-GetMaxNetFrameworkVersion-null-null", "KeePass.Plugins.PlgxPlugin");  
					if((uInst != 0) && (uReq > uInst))
						throw new PlgxException(KPRes.NewerNetRequired);
				}
				else if(kvp.Key == PlgxPrereqOS)
				{
					string s = (string) Router.ForwardCall("KeePass.Util.WinUtil-GetOSStr-null-null", "KeePass.Plugins.PlgxPlugin");
					string strOS = "," + s + ",";
		...
		private static string CreateTempDirectory()
		{
			...
			string strTmpRoot = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.Plugins.PlgxPlugin");
			...
			string strBuildPre, string strBuildPost)
		{
			...
			string s = (string) Router.ForwardCall("KeePass.Util.WinUtil-GetExecutable-null-null", "KeePass.Plugins.PlgxPlugin");
