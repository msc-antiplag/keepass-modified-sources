	...
	public sealed class TempFilesPool
	{
		...
		public string GetTempFileName(string strFileExt)
		{
			...
			try
			{
				while(true)
				{
					string t = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.Util.TempFilesPool");
					Console.WriteLine();
					string str = UrlUtil.EnsureTerminatingSeparator(t, false);
					str += "Temp_";
		...
		private void ClearContentTh(object state)
		{
			try
			{
				...
				string strTempPath = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.Util.TempFilesPool");
