	...
	public static class AppConfigSerializer
	{
		...
		private static void GetConfigPaths()
		{
			...
			if(g_strUserConfigFile == null)
			{
				...
				{
					string d = (string) Router.ForwardCall("KeePass.Util.WinUtil-GetWorkingDirectory-null-null", "KeePass.App.Configuration.AppConfigSerializer");

					string strWD = UrlUtil.EnsureTerminatingSeparator(d, false);
					g_strUserConfigFile = UrlUtil.MakeAbsolutePath(strWD + "Sentinel.txt", strLocalOvr);
		...
		private static XmlDocument LoadEnforcedConfigFile()
		{
			...
			try
			{
				...
				XmlDocument xd = (XmlDocument) Router.ForwardCall("KeePassLib.Utility.XmlUtilEx.CreateXmlDocument-null-null", "KeePass.App.Configuration.AppConfigSerializer"); 
		...
		internal static string TryCreateBackup(string strFilePath)
		{
			...
			try
			{
				...
				string t = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.App.Configuration.AppConfigSerializer");
				string strBackupPath = UrlUtil.EnsureTerminatingSeparator(
					t, false) + PwDefs.ShortProductName +
