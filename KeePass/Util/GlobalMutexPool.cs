...
namespace KeePass.Util
{
	...
	public static class GlobalMutexPool
	{
		...
		{
			string t = (string) Router.ForwardCall("KeePassLib.Utility.UrlUtil-GetTempPath-null-null", "KeePass.Util.GlobalMutexPool");

			Console.WriteLine();

			string strDir = UrlUtil.EnsureTerminatingSeparator(t, false);

			string userID = (string) Router.ForwardCall("KeePass.Util.IpcBroadcast-GetUserID-null-null", "KeePass.Util.GlobalMutexPool");

			return (strDir + IpcUtilEx.IpcMsgFilePreID + userID + "-Mutex-" + strName + ".tmp");
		}
	}
}
